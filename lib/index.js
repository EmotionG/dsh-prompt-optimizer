...
async function buildContextLines(sessionQuery, sessionId) {
    const lines = [];
...
export function apply(ctx) {
        ...
        try {
            connection.register(scoped, RPC_CHANNEL, async (endpoint, payload, signal) => {
                ...
                if (endpoint === 'cancel') {
                    ...
                    if (run)
                        run.abort('prompt-optimizer: canceled by caller');
                ...
                if (token !== '')
                    activeRuns.set(token, controller);
    ...
    ctx.effect(() => () => {
        for (const controller of activeRuns.values())
            controller.abort('prompt-optimizer: plugin stopped');
        activeRuns.clear();
